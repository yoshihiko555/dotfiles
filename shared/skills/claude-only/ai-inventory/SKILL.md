...
### Step 6: 月次レポートページを作成する

AI-ASSETSの親ページ（`1416d81aac528070960ac832992675a5`、Workflowハブ）配下に `棚卸しレポート YYYY-MM` ページを `create-pages` で作成する。本文に以下を含める。
...
作成後、TASK DB（データソースID: `collection://1316d81a-ac52-81be-b589-000baec0b7a6`。参照: `shared/skills/common/notion-task/references/notion-schema.md`）を `query-data-sources` で「タイトルが『月次棚卸_AI資産レビュー』＋当月で前方一致」の条件で検索する。

- 一意に決まれば、そのタスクページ本文にレポートへのリンクを `update-page`（`insert_content`）で追記する
- 0件または複数件ヒットした場合は、候補を提示してユーザーに選ばせる（推測で1件に決め打ちしない）
