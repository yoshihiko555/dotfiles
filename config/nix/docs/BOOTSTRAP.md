...
グループ無しでそのまま復元できる。`add_new_trigger` は `triggers.json` の
`BTTUUID` をそのまま採用するので、復元後に再度 apply しても重複しない。

初回の apply は「repo と実機の両方に同じ UUID があって中身が違う」場合だけ
拒否する。まっさらな BTT（トリガー 0 件）や、実機にしか無いトリガーがある
状態からは、そのまま追加が走る。
