...
- ステータス: **一部改訂**（[ADR-20260730-0003](ADR-20260730-0003-purpose-and-order.md) を参照）
- 決定日: 2026-07-29
- 関連: [ADR-20260424-0001](ADR-20260424-0001-nix-layout.md)（ディレクトリ構成）

> ## ⚠️ 改訂注記（2026-07-30）
>
> 本 ADR には**事実誤認**があり、以下は
> [ADR-20260730-0003](ADR-20260730-0003-purpose-and-order.md) で改訂済み。
> 記録として原文は残す。
>
> | 本 ADR の記述 | 実態・改訂後 |
> |---|---|
> | 「WSL2 上で日常的に開発している」 | **誤り**。WSL2 は予備機で、現状は稼働プロジェクトがなく日常的に使用していない。普段の開発は MacBook Pro が主流 |
> | 「トリガー 3 つ全点灯」 | 「Linux を日常的に触る」は**現時点では点灯していない** |
> | 着手順序 `WSL2 → hermes → MacBook Pro` | **反転**して `hermes → MacBook Pro → WSL2`。WSL2 は実稼働待ちの条件付き |
> | nix-darwin は Phase 4（未判断） | **最初から使う**（`homebrew.onActivation.cleanup = "zap"` が目的） |
> | 方式 D1 を「A に全面固定」 | 「**原則 + 例外**」に修正。循環参照の罠があり、複雑なケースは `home.activation` に落ちる |
>
> なお、D1 の中核（設定ファイルを Nix 言語で書き直さず `mkOutOfStoreSymlink` を主軸にする）と、
> ADR-20260424-0001 のディレクトリ構成は**妥当性が裏付けられた**（mozumasu/dotfiles の実装で確認）。
