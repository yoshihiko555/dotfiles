...
ウィンドウを新しく開くと、アプリを問わず、移動先ワークスペースのタイル型ウィンドウを
自動整列する。異なるアプリが混在していても対象になる。
...
| 3 | 左に 1 枚、右に上下 2 枚 |
| 4 | 2×2 の均等な配置 |
| 5 以上 | 自動整列せず、通常の AeroSpace の配置に任せる |

既存のアプリ別 WS 振り分けを適用した後に整列し、フォーカスは移動しない。
フローティングのウィンドウはアプリを問わず対象外とし、枚数にも含めない。
並びはウィンドウ ID の昇順で、4 枚なら左上・左下・右上・右下になる。
