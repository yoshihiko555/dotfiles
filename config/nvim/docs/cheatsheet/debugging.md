...
VSCode 互換の F キーも使える。`F9` ブレークポイント切替、`F5` 開始・続行、
`F10` ステップオーバー、`F11` ステップイン、`F12` ステップアウト。
VSCode のステップアウトは Shift+F11 だが、tmux が `<F23>` に変換して Neovim に
`<S-F11>` として届かないため F12 に割り当てている。`<leader>d` 系はそのまま残っている。
