...
画面上部（lualine の tabline）の左側に、開いているバッファが番号付きで並ぶ。

| キー | 説明 |
|------|------|
| `Shift+h` / `Shift+l` | 前 / 次のバッファ |
| `<leader>1`〜`<leader>9` | 上部に表示された番号のバッファへ |
| `<leader>bn` | 次のバッファ |
| `<leader>bp` | 前のバッファ |
| `<leader>bd` | バッファ削除 |
| `<leader>fb` | バッファを fzf で絞り込んで開く |

標準操作も併用できる:

| キー / コマンド | 説明 |
|------|------|
| `Ctrl+^` | 直前のバッファへ戻る（上部で `#` が付いているバッファ） |
| `[b` / `]b` | 前 / 次のバッファ |

## タブ (`<leader>t`)

ファイルの切り替えはバッファで行い、タブは分割レイアウトを崩さずに別の画面（差分レビュー・デバッグなど）を一時的に開くときに使う。
タブが 2 つ以上あるとき、画面上部の右側にティール色で番号（名前を付けたタブは名前も）が並ぶ。ファイル名は出さない。
...
| キー | 説明 |
|------|------|
| `<leader>ub` | inline blame トグル（git 管理下のファイルのみ） |
| `<leader>uw` | word diff トグル（git 管理下のファイルのみ） |
