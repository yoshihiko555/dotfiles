# ADR-004: Prefix キーの変更 (Ctrl+B → Ctrl+T → Ctrl+Q)

- **状態**: 承認・実施済み
- **日付**: 2026-03-17
- **更新**: 2026-03-17 Phase 6 完了に伴い Ctrl+T → Ctrl+Q に再変更
...
- `keybinds.conf` の Prefix を `C-q` に変更
- WezTerm `keybinds.lua` の Cmd→tmux 変換を `Ctrl+Q` ベースに変更
- `.zshrc` の `stty -ixon` は引き続き必要 (Ctrl+Q の XON/XOFF 解放のため)

## 更新履歴

1. **初回 (Phase 1)**: WezTerm Leader (`Ctrl+Q`) との衝突を避けるため `Ctrl+T` を採用
2. **Phase 6 完了後**: WezTerm Leader を廃止したため `Ctrl+Q` に再変更。押しやすさを優先
